...
module.exports = async (pluginConfig, context) => {
  ...
  if (publishCmd === false) {
    ...
    await exec(publishCmd ? publishCmd:'npm run release', context);
  }


  debug('release object: %O', release);

  const re = (await github.repos.listReleases(pick(release, ["owner", "repo"])))
      .data
      .find(x => x.tag_name === gitTag);
  if (!re) {
    throw new SemanticReleaseError("created draft github release not found!!\nPlease check electron-builder publish commnad");
  }
  
  const { html_url: url, upload_url: uploadUrl, id: releaseId } = re;
