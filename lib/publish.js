...
module.exports = async (pluginConfig, context) => {
  ...
  const {githubToken, githubUrl, githubApiPathPrefix, proxy, assets, publishCmd} = resolveConfig(pluginConfig, context);
  ...
  if (publishCmd === false) {
    ...
    await exec(publishCmd ?? 'npm run release -- -c.releaseInfo.releaseNotes=${nextRelease.notes}', context);
  }
  ...
  const {html_url: url, upload_url: uploadUrl, id: releaseId  } = (await github.repos.listReleases(pick(release, ["owner", "repo"])))
      .data
      .find(x => x.tag_name === release.tag_name);
